...
import {
  ...
  PaymentStatus,
  PropertyTypes,
} from "../db/enums.schema.ts";

// Convert enum objects to arrays for the template
const highlightsArray = JSON.stringify(Object.values(Highlights));
const amenitiesArray = JSON.stringify(Object.values(Amenities));
const propertyTypesArray = JSON.stringify(Object.values(PropertyTypes));
const paymentStatusArray = JSON.stringify(Object.values(PaymentStatus));
const applicationStatusArray = JSON.stringify(Object.values(ApplicationStatus));

const template = `// Auto-generated from backend Drizzle schema
...
// ============ Enum Schemas ============
export const highlightSchema = z.enum(${highlightsArray});
export const amenitySchema = z.enum(${amenitiesArray});
export const propertyTypeSchema = z.enum(${propertyTypesArray});
export const paymentStatusSchema = z.enum(${paymentStatusArray});
export const applicationStatusSchema = z.enum(${applicationStatusArray});
...
const frontendPath = resolve(
  __dirname,
  "../../../frontend/src/types/dbTypes.d.ts"
);
mkdirSync(dirname(frontendPath), { recursive: true });
writeFileSync(frontendPath, template);

console.log("✅ Types and schemas generated at:", frontendPath);
